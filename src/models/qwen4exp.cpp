...
void llama_model_qwen4exp::load_arch_hparams(llama_model_loader & ml) {
    ...
    // MTP draft head: the number of nextn-predict layers past the trunk (0 for a plain model)
...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    // An MTP-only file (the standalone draft head) carries just the draft block. Keep
    // walking the trunk so the per-layer bookkeeping still runs, but let its tensors be
...
llama_model_qwen4exp::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    // reads the FULL per-token state (a flat copy of [n_embd, hc, T]); the logits path
    // gathers only the output rows. Mirrors deepseek4's MTP handoff so t_h_nextn is sized
    // n_tokens (the shared read uses n_tokens), not n_outputs.
...
ggml_tensor * llama_model_qwen4exp::graph::build_layer_attn(
    ...
    // a null mctx_hyb means no QSA indexer (e.g. the single-block MTP draft), so attend dense
