...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_cpy_rot(ggml_metal_library_t lib, ggml_type tsrc, ggml_type tdst) {
    char base[256];
    char name[256];

    // rotated-domain TBQ dequant (centroids only, no inverse WHT) — used by the
    // attention fallback on backends without a fused TBQ FA kernel
    snprintf(base, 256, "kernel_cpy_%s_%s_rot", ggml_type_name(tsrc), ggml_type_name(tdst));
    snprintf(name, 256, "%s", base);
